#' find the pixels in a sector area and write them into a csv file to run xstilt as receptors
#' @author dogtorX
...
recp_fn              = sys.argv[5]
OCO3_data_file       = sys.argv[6]
OCO3_obs_file        = sys.argv[7]
fig_demo_file        = sys.argv[8]

YYYYMN     = timestr[0:6]
month      = timestr[2:6]
date       = timestr[4:8]

plume = pd.read_csv(segmentation_file)
domain_opt = []
...
def main():
    ...
    lat_lon_data = filter_pair(domain_opt, recp_fn, OCO3_data_file)
    pic_map(lat_lon_data, date, fig_demo_file)
    return 0

def filter_pair(domain_opt, recp_fn, OCO3_data_file):
    ...
    lat_lon_data.to_csv(OCO3_data_file,sep=',',index=0, header=1)

    lat_lon = pd.DataFrame({'lati':lat_lon_data['lat'], 'long':lat_lon_data['lon']})
    lat_lon.to_csv(recp_fn,sep=',',index=0, header=1)

    return lat_lon_data


def pic_map(lat_lon_data, date, fig_demo_file):
