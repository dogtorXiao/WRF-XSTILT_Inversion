#' run xstilt with wind error and generate footprints
#' @author DogtorX'

run_wind = function(namelist){
  ...
  store_path = file.path(xstilt_wd, 'XSTILT_output_wind')
  ...
  run_wind_err  = T    # T: calc wind error based on RAOB 
  ...
  if (run_hor_err) {
    ...
    ctflux_path = file.path(ct_path, 'flux/daily')
    ctmole_path = file.path(ct_path, 'molefractions_co2_total')
