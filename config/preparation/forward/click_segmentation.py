#' select the pixels inside the downwind domain
#' @author DogtorX
...
forward_fig_file      = sys.argv[1]
segmentation_file     = sys.argv[2]

# function delaing with button
def close_figure(event):
    plt.close(fig)

# read image
img = plt.imread(forward_fig_file)

# create a new image window
fig, ax = plt.subplots()
plt.subplots_adjust(bottom=0.2) # save space for close button
...
# input the lon, lat, as refernces
...
lon_lat_seg.to_csv(segmentation_file, index=False)